...
//		this.songSelector = playlist.listIterator();
	...
	public void addSong(T songName) {
		...
		} else if (playlist.add(songName))
			System.out.println("Successfully added \"" + songName + "\" to the playlist.");
	}

	public void playSong() {
		if(!playlist.isEmpty()) {
			this.songSelector = playlist.listIterator();
			System.out.println(songSelector.next());;
			playlist.remove();
//			System.out.println("Playing \"" + songSelector.next() + "\".");
//			while(songSelector.hasNext()) {
//				System.out.println(songSelector.next());;
//			}
//			songSelector.next();
