...
public class SceneKitchen extends OpeningScene {
	...
	public SceneKitchen() {
		...
		this.eventOptions = new String[] {
				"A. Look through the garbage on the counter for something useful.\n" + "B. Open the refrigerator." };
	}
	...
	public void sceneExitAction(BaseCharacter playerOne) {
		...
		case "A": {
			...
			if (playerOne.getIntelligence() > 8) {
				...
				playerOne.gainLevel();
				System.out.println("Amongst all the rot, you somehow manage to find something medicinal.");
		...
		case "B": {
			...
			if (playerOne.getStrength() > 8) {
				System.out.println("You shrink back at what you find. You immediately slam it shut.");
				System.out.println("You move on.");
